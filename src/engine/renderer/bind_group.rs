...
impl LayoutBuilder {
    ...
    pub fn build(&mut self, label: &str) -> LayoutInfo {
        return LayoutInfo {
            layout: self
                .device
...
impl<'a> ResourceBuilder<'a> {
    pub fn new(device: &wgpu::Device, layout: &'a LayoutInfo) -> Self {
        return Self {
            device: device.clone(),
            layout,
            entries: Vec::new(),
        ...
        match expect.ty {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "incorrect binding resource order. buffer is not next"
                )
                .into());
            }
        }
    ...
    pub fn texture_view(&mut self, buffer: &'a wgpu::TextureView) -> anyhow::Result<&mut Self> {
        ...
        match expect.ty {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "incorrect binding resource order. texture view is not next"
                )
                .into());
            }
        }
    ...
    pub fn texture_storage(&mut self, buffer: &'a wgpu::TextureView) -> anyhow::Result<&mut Self> {
        ...
        match expect.ty {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "incorrect binding resource order. texture storage is not next"
                )
                .into());
            }
        }
    ...
    pub fn texture_sampler(&mut self, buffer: &'a wgpu::Sampler) -> anyhow::Result<&mut Self> {
        ...
        match expect.ty {
            ...
            _ => {
                return Err(anyhow::anyhow!(
                    "incorrect binding resource order. texture sampler is not next"
                )
                .into());
            }
        }
